...
func (p *Password) defaultArgonParams() *ArgonParams {
	return &ArgonParams{
		Memory:      64 * 1024,
		Iterations:  3,
		Parallelism: 2,
...
// Hash will generate a hash of a password ready to be stored in the database
func (p *Password) Hash(password string) (string, error) {
	params := p.defaultArgonParams()
...
// Verify will check for the validity of the password in the database
func (pwd *Password) Verify(password, encodedHash string) (match bool, err error) {
